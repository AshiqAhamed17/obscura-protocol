                ...
                <select value={selected} onChange={(e) => setSelected(Number(e.target.value))}>
                  {notes.map((n, i) => (
                    <option key={i} value={i}>
                      Market #{n.marketId.toString()} · outcome {n.side} · {usdc(n.amount)}
                    </option>
                  ))}
                </select>
              </div>
...
function ClaimForm({ note, recipient }: { note: Note; recipient: `0x${string}` }) {
  ...
  const outcomeTotals = (outcomeTotalsData as readonly bigint[] | undefined) ?? [];

  const isSettled = marketStatus === 2;
  const onWinningSide = winningOutcome !== undefined && note.side === winningOutcome;
  const alreadySpent = spent === true;

  // proportional (pari-mutuel) payout from the now-public proven per-outcome totals
  const winningTotal = winningOutcome !== undefined ? (outcomeTotals[winningOutcome] ?? 0n) : 0n;
  const payout = market && onWinningSide && winningTotal > 0n ? (note.amount * market.totalPool) / winningTotal : null;
  ...
  async function claim() {
    ...
    try {
      ...
      if (market && cp.computedRoot.toLowerCase() !== market.merkleRoot.toLowerCase()) {
  ...
  return (
    <>
      ...
      <div className="note">
        ...
        {market && market.feed !== "0x0000000000000000000000000000000000000000" && ` · ${feedLabel(market.feed).asset} ≥ ${usd(market.threshold)}`}
        {isSettled && market && ` · winning: ${outcomeLabel(winningOutcome!, market.numOutcomes)}`}
      </div>
      ...
      {isSettled && onWinningSide && payout !== null && (
        <div className="payout-est">
          ...
          <b className="payout-v tag-yes">{usdc(payout)}</b>
        </div>
      )}
